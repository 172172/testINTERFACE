...
const [questions,parties,positions,meta,sourceRegistry]=await Promise.all(['questions','parties','positions','meta','source-register'].map(n=>fetch(`data/${n}.json`).then(r=>r.json())));
...
function renderAudit(){
  ...
  $('#app').innerHTML=`<section><div class="eyebrow">Transparens</div><h1>Granska kompassen</h1><p class="lead">Här finns frågorna, varje kodad partiposition, källan, verifieringsdatumet och osäkerhetsgraden. ${known} av ${total} möjliga parti–fråga-positioner är verifierade i denna forskningspreview.</p><div class="actions"><button class="button" id="json">Exportera JSON</button><button class="button secondary" id="csv">Exportera CSV</button></div><h2>Officiella källingångar</h2><div class="grid grid-3">${sourceRegistry.map(s=>`<article class="card"><strong>${esc(byP[s.party]?.name||s.party)}</strong><p><a href="${esc(s.url)}" target="_blank" rel="noopener">${esc(s.title)} ↗</a></p><p class="muted">${esc(s.note)}</p></article>`).join('')}</div><div class="table-wrap" style="margin-top:1rem"><table><thead><tr><th>Fråga</th><th>Avgränsning</th><th>Område</th><th>Parti</th><th>Position</th><th>Säkerhet</th><th>Källa / motivering</th><th>Verifierad</th></tr></thead><tbody>${positions.map(p=>{const q=byQ[p.question],party=byP[p.party];return `<tr><td>${esc(q.text)}</td><td>${esc(q.scope||'')}</td><td>${esc(q.area)}</td><td>${esc(party.name)}</td><td>${p.position===null?'<span class="unknown">Okänd</span>':`<span class="known">${esc(posLabel(p.position))}</span>`}</td><td>${esc(p.confidence)}</td><td>${p.source?`<a href="${esc(p.source)}" target="_blank" rel="noopener">${esc(p.sourceTitle||'Officiell källa')}</a><br>`:''}<span class="muted">${esc(p.rationale)}</span></td><td>${esc(p.verified)}</td></tr>`}).join('')}</tbody></table></div></section>`;
  $('#json').onclick=()=>download('valkompass-positions.json',JSON.stringify({meta,questions,parties,positions,sourceRegistry},null,2),'application/json');
  $('#csv').onclick=()=>{const header=['party','question','area','statement','scope','position','confidence','source','sourceTitle','sourceDate','verified','rationale'];const csv=[header,...positions.map(p=>[byP[p.party].name,p.question,byQ[p.question].area,byQ[p.question].text,byQ[p.question].scope??'',p.position??'',p.confidence,p.source??'',p.sourceTitle??'',p.sourceDate??'',p.verified,p.rationale])].map(r=>r.map(v=>`"${String(v).replaceAll('"','""')}"`).join(',')).join('\n');download('valkompass-positions.csv',csv,'text/csv;charset=utf-8')};
}
function download(name,text,type){const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([text],{type}));a.download=name;a.click();URL.revokeObjectURL(a.href)}

function renderMethod(){
  $('#app').innerHTML=`<section><div class="eyebrow">Metod</div><h1>Exakt så räknar kompassen</h1><div class="grid grid-2"><article class="card"><h2>Skalor</h2><p>Användarsvar och partiposition kodas −2, −1, 0, +1, +2. Avståndet för fråga <span class="mono">i</span> är <span class="mono">dᵢ = |uᵢ − pᵢ|</span>. Största möjliga avstånd är 4.</p><p>Vikt anges 0–5. Vikt 0 påverkar inte prioritetsmatchningen.</p></article><article class="card"><h2>Total matchning</h2><p class="mono">100 × (1 − Σ dᵢ / (4 × N))</p><p>Alla besvarade frågor med verifierad partiposition väger lika. <span class="mono">N</span> är antalet jämförbara frågor.</p></article><article class="card"><h2>Prioritetsmatchning</h2><p class="mono">100 × (1 − Σ wᵢdᵢ / (4 × Σ wᵢ))</p><p>Dina egna vikter <span class="mono">wᵢ</span> avgör hur mycket varje fråga betyder.</p></article><article class="card"><h2>Saknade positioner</h2><p>En okänd position tas inte med i täljare eller nämnare. Dessutom visas källtäckning separat. Full ranking kräver minst ${(meta.rankingCoverageThreshold*100).toFixed(0)} % viktad täckning.</p></article></div><h2>Frågedesign</h2><p>Frågebanken innehåller ${questions.length} frågor i ${new Set(questions.map(q=>q.area)).size} politikområden, fyra frågor per område. Varje fråga ska mäta en konkret politisk skiljelinje och har en separat avgränsning som anger exakt vad som ingår och inte ingår. Bredare formuleringar som exempelvis ‘sänk skatten på arbete’ används inte när partier kan mena olika målgrupper eller skatteinstrument.</p><p>Partinamn visas inte under testet och inga löpande matchningsresultat visas.</p><h2>Källhierarki</h2><ol><li>Officiellt valmanifest eller partiprogram.</li><li>Partiets officiella webbplats.</li><li>Partiets egna svar i etablerade valkompasser.</li><li>Propositioner, motioner och voteringar.</li><li>Tydliga uttalanden från partiledning eller officiella företrädare.</li></ol><div class="callout"><strong>Ingen AI-positionering.</strong> Kodningen är en datapost som måste bära sin egen källa, motivering, datum och osäkerhetsgrad. En källa måste svara på den exakta avgränsningen i frågan. Om partiet bara stödjer en närliggande men annorlunda reform får det inte automatiskt samma position. Om underlaget inte räcker sätts <span class="mono">position: null</span>.</div></section>`;
}
route();
